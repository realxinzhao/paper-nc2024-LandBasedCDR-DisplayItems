MainFig3_LandCarbonPricingStrength_Updated <- function(){
  ...
  LandToEMs_decompose <- LoadFigData("LandToEMs_decompose_Updated")
  ...
  # Fig. F ----
  LandToEMs_decompose <- LoadFigData("LandToEMs_decompose_Updated") %>%  filter(LandSupply == "2C Main")
  ...
  ggplot() +
    ...
    scale_linetype_manual(values = c(2, 1, 4, 3), name = "Land removal intensity") +
    theme_bw() + theme0 +
    theme(panel.grid = element_blank(),
          legend.text.align = 0,
          strip.background = element_blank(),
  ...
  plot_grid(FigE1 +  theme(legend.position = "none", plot.title = element_blank()),
            ...
            legendF1, nrow = 1,
            rel_widths = c(0.24, 0.75, .21),
            align = c('hv'), axis = "tb") -> FigEF0

  ggdraw() + draw_label("(E) Land removal intensity", size = 30,
                        fontface = 'bold', x = 0, hjust = 0) +
    theme(plot.margin = margin(t=5, b=10, l = 20)) -> FigE1Title

  ggdraw() + draw_label("(F) Decomposition of land removal intensity", size = 30,
                        fontface = 'bold', x = 0, hjust = 0) +
    theme(plot.margin = margin(t=5, b=10, l = 10)) -> FigF1Title

  plot_grid(
    FigE1Title, FigF1Title, NULL, ncol = 3,
    rel_widths =  c(0.24, 0.75, 0.21)
  ) -> FigEFTitles
